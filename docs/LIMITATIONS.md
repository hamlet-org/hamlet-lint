...
### 1.2 Hand-rolled wrappers that perfectly mimic PPX-generated `Tag.give`

The recursive-residual `provide` detector (`extract/propagate.ml`)
identifies real PPX-generated `Tag.give` via a structural fingerprint
on the callee's `val_type`: codomain is `Hamlet.Dispatch.t`, first
parameter is annotated as `r` (a `Tconstr` resolving to some
`<...>.r`). PPX-generated `Tag.give` always satisfies both;
unannotated user wrappers fail the first-arg check and fall back.

A user could *deliberately* write a wrapper that satisfies both:

```ocaml
module Tag = struct
  type r = Console.Tag.r
  let give (w : r) _ = Hamlet.Dispatch.need w  (* re-emits, not discharges *)
end
```

Used as the inner step of a chained inline `provide`, this wrapper is
indistinguishable from real `Tag.give` at the typedtree level — the
linter would classify it as a discharge and incorrectly narrow the
inner residual, potentially producing a bogus outer finding.

**Why we don't fix this:** the only signal that distinguishes
"PPX-generated" from "user-replicated" is the source file location of
the macro expansion, which `vd.val_loc` does not preserve in a way
the linter can reliably pattern-match. Any structural fingerprint can
be replicated by sufficiently-determined user code. The shape above
is also wildly non-idiomatic — real Hamlet code never hand-rolls
`Tag` modules; they're always generated by `[%hamlet.service]`.

**Workaround:** if your project genuinely needs a `Tag.give` wrapper
for some reason, let-bind every step of the chain so that the inner
step's residual is read off `val_type` directly (the trick from §1.1
workaround), bypassing the recursive detection entirely.
