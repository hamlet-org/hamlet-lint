...
Install a matching Hamlet and subtractor release pair:

```sh
opam install hamlet hamlet-subtractor
```

`hamlet-subtractor` is tied to a specific OCaml compiler-libs generation. Each
release pins one exact Hamlet and OCaml patch pair. Compatibility begins at
OCaml 5.5.0, and the current matrix contains 5.5.0. Dune 3.18 or newer is
required. This repository tests the editor path with Merlin 5.7 for OCaml
5.5.0 and OCaml-LSP 1.26.
