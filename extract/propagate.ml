        ...
        if not first_arg_is_var then Pa_unknown
          ...
          | Texp_ident (path, _, _) -> (
              ...
              | Some tags ->
                  ...
                  else if Path.last path = "give" then
                    (* heuristic: a function called "give" on a Tag-like
                       module path. The provide handler context already
                       guarantees we're inside a Hamlet provide (caller
                       classified the outer combinator), so this is
                       sufficient without re-fingerprinting. *)
                    Pa_give tags
                  else Pa_unknown)
          | _ -> Pa_unknown)
    | _ -> Pa_unknown
